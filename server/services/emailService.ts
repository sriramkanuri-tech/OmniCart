...
export const sendOrderConfirmationEmail = async (orderData: any): Promise<boolean> => {
  try {
    const { 
      orderId, uropayTransactionId, productName, amount, 
      category, userEmail, userName, placedAt, status 
    } = orderData;
  ...
  const dateStr = placedAt ? new Date(placedAt).toLocaleString("en-IN", { timeZone: "Asia/Kolkata" }) : new Date().toLocaleString("en-IN", { timeZone: "Asia/Kolkata" });
      ...
      html: `
        <div style="font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; max-width: 600px; margin: 0 auto; color: #1e1b4b; background-color: #fcfbfe; border-radius: 16px; overflow: hidden; border: 1px solid #e9e3ff;">
          ...
          <div style="background-color: #f5f3ff; padding: 24px; text-align: center; font-size: 12px; color: #6b7280; border-top: 1px solid #e9e3ff;">
            ...
            <p style="margin: 0;">&copy; ${new Date().getFullYear()} OmniCart Inc. All rights reserved.</p>
          </div>
        </div>
      `,
    };
