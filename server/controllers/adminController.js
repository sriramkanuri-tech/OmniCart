...
export const adminController = {
  ...
  getStats(req, res) {
    ...
    const todayOrdersList = allOrders.filter((o) => {
      const orderDate = o.placedAt || (/* @__PURE__ */ new Date()).toISOString();
      return orderDate.substring(0, 10) === todayStr;
    });
    const todayOrders = todayOrdersList.length;
    const todayRevenue = todayOrdersList.filter((o) => o.status === "PAID" || o.status === "PLACED").reduce((sum, o) => sum + o.amount, 0);
    const categoryMap = {
      Shopping: 0,
      Food: 0,
      Travel: 0,
    ...
    res.json({
      ...
      // fallback so visuals aren't fully blank
