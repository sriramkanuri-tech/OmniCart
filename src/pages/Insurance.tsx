...
import { 
  Shield, Lock, FileText, Landmark, Search, ArrowRight, HeartHandshake, 
  Eye, Briefcase, Car, Home, Heart, X, CheckCircle2, RefreshCw, Send, AlertTriangle
} from 'lucide-react';
...
export default function Insurance() {
  ...
  const handlePlaceClaim = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleBuyPolicyItem = (policy: any) => {
  ...
  const handleWizardBuy = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const POLICY_TYPES = [
    { icon: Home, title: 'Home Insurance', val: '₹ 8.5Cr Comprehensive Cover', price: 15000, color: 'bg-blue-50 text-blue-600' },
    { icon: Car, title: 'Motor Insurance', val: 'Fully Active Zero-Dep Cover', price: 8500, color: 'bg-orange-50 text-orange-600' },
    { icon: Heart, title: 'Life Insurance', val: '₹ 2.5Cr Wealth Trust Cover', price: 22000, color: 'bg-red-50 text-red-600' },
    { icon: Briefcase, title: 'Business Policy', val: 'Elite Premium Liability Cover', price: 45000, color: 'bg-green-50 text-green-600' }
  ];
    ...
    <div className="space-y-12 pb-12 font-sans relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <button 
          ...
          className="flex items-center gap-2 px-6 py-3 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 group transition-all cursor-pointer active:scale-95"
        >
      ...
      <section className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {POLICY_TYPES.map((item, i) => (
            <motion.div 
              key={i} 
              whileHover={{ y: -8 }}
              onClick={() => setSelectedPolicy(item)}
              className="bg-white p-10 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all group cursor-pointer flex flex-col justify-between h-full"
            >
               <div>
      ...
      <section className="bg-white rounded-[3rem] border border-gray-100 shadow-sm overflow-hidden animate-fade-in">
        ...
        <div>
          {[
            { title: 'Quarterly Risk Evaluation Report', type: 'Confidential Risk Audit', verified: 'By KPMG' },
            { title: 'Asset Validation Certification - March', type: 'Authorized Asset Ledger', verified: 'By Lloyds Registry' },
            { title: 'Estate Liability Coverage Briefing', type: 'Private Wealth Trust Annex', verified: 'By Sovereign Trust' },
            { title: 'Policy Renewal Payment Receipt', type: 'Settle Invoice Statement', verified: 'By OmniSettle Engine' }
          ].map((doc, i) => (
            <div 
              key={i} 
              onClick={() => setSelectedDoc(doc.title)}
              className="flex items-center justify-between p-8 hover:bg-gray-50 transition-all cursor-pointer group border-b border-gray-50 last:border-0"
            >
        ...
        {selectedPolicy && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
            ...
            >
              ...
              <button 
                onClick={() => handleBuyPolicyItem(selectedPolicy)}
              ...
              <form onSubmit={handlePlaceClaim} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <select 
                    ...
                    className="w-full bg-gray-50 border border-gray-100 p-4 rounded-xl text-xs font-bold outline-none font-sans"
                  >
                ...
                <div className="space-y-2">
                  ...
                  <input 
                    ...
                    className="w-full bg-gray-50 border border-gray-100 p-4 rounded-xl text-sm font-bold outline-none font-sans font-mono"
                    placeholder="25000"
                  />
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <textarea 
                    ...
                    className="w-full h-24 bg-gray-50 border border-gray-100 p-4 rounded-xl text-xs font-semibold outline-none font-sans resize-none"
                    placeholder="Provide incident timestamps & damage attributes..."
                  />
                </div>

                <button 
                  type="submit"
                  className="w-full h-14 bg-orange-500 hover:bg-orange-600 text-white rounded-2xl font-bold text-sm shadow-xl shadow-orange-100 flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer"
                >
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="space-y-4">
                {[
                  { name: 'Vehicle Auto-Renew Shield', val: 'Active • Renewal Oct 2026', key: 'veh' },
                  { name: 'Home Security Asset Cover', val: 'Active • Auto-Renew OFF', key: 'hom' },
                  { name: 'Global Dental & Health Rider', val: 'Active • Renewal Dec 2026', key: 'hlth' }
                ].map((pol, idx) => (
                  <div key={idx} className="p-4 bg-gray-50 border border-gray-100 rounded-2xl flex justify-between items-center">
                    ...
                    <button 
                      onClick={() => triggerToast(`Altered automatic settlement configurations for "${pol.name}"`)}
                      className="p-2.5 hover:bg-purple-55 text-gray-400 hover:text-[#7C3AED] rounded-xl border border-gray-100"
                    >
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="flex gap-4">
                ...
                <button 
                  onClick={() => setSelectedDoc(null)}
                  className="h-14 px-5 bg-gray-100 hover:bg-gray-200 text-gray-600 rounded-2xl font-bold text-xs"
                >
                  Close
                </button>
              </div>
