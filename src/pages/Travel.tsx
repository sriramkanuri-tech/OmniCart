...
import { 
  Plane, Hotel, Train, Bus, MapPin, Calendar, Users, 
  ArrowRight, ArrowLeftRight, Compass, ShieldCheck, Star, X, CheckCircle2,
  Armchair, AlertCircle, Plus, Edit2
} from 'lucide-react';
...
const FLIGHTS = [
  { id: 'EK-503', carrier: 'Emirates', dep: '10:30 AM', arr: '02:45 PM', price: 42000, duration: '4h 15m' },
  { id: 'AI-101', carrier: 'Air India', dep: '06:00 AM', arr: '11:15 AM', price: 18500, duration: '5h 15m' },
  { id: 'SQ-421', carrier: 'Singapore Airlines', dep: '09:15 PM', arr: '03:30 AM', price: 34000, duration: '6h 15m' },
];

const TRAINS = [
  { id: '12002', carrier: 'Shatabdi Express', dep: '06:00 AM', arr: '10:45 AM', price: 1200, duration: '4h 45m' },
  { id: '12952', carrier: 'Rajdhani Express', dep: '04:30 PM', arr: '08:15 AM', price: 2800, duration: '15h 45m' },
  { id: '22691', carrier: 'Rajdhani Premium AC', dep: '08:00 PM', arr: '11:30 AM', price: 4100, duration: '15h 30m' },
];

const BUSES = [
  { id: 'KA-9010', carrier: 'KSRTC Ambaari Sleeper', dep: '09:00 PM', arr: '06:00 AM', price: 1600, duration: '9h 00m' },
  { id: 'SR-441', carrier: 'SRS Travels Multi-Axle', dep: '10:15 PM', arr: '07:30 AM', price: 1450, duration: '9h 15m' },
  { id: 'VRL-772', carrier: 'VRL Sleeper Coach', dep: '07:15 PM', arr: '05:45 AM', price: 1800, duration: '10h 30m' },
];

export default function TravelPage() {
  const { balance, updateBalance, addTransaction, addOrder, profile } = useGlobalState();
  const isAdmin = profile?.email === "sriramkanuri4@gmail.com";
  const [trendingList, setTrendingList] = useState<any[]>(TRENDING);
    ...
    const fetchTravelData = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) {
          const travelProducts = res.data.filter((p: any) => p.category === 'Travel');
          if (travelProducts.length > 0) {
            const customTravelList = travelProducts.map((p: any) => ({
              id: p.id,
              name: p.name,
  ...
  const [bookedSeats, setBookedSeats] = useState<string[]>(['1C', '2A', '3D', '5B', 'LB-3', 'UB-8', 'W-2', 'A-5']);
  ...
  return (
    <div className="space-y-12 pb-12 font-sans relative">
           ...
           {([
             ...
             <button 
               ...
               className={cn(
                 ...
                 activeTab === tab.id ? "bg-[#7C3AED] text-white shadow-lg" : "text-gray-500 hover:bg-gray-50"
               )}
             >
               {tab.label}
             </button>
      ...
      <section className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-xl space-y-10">
         <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            <div className="space-y-4">
               ...
               <div className="relative group">
                  <input 
                    ...
                    className="w-full bg-gray-50 border border-gray-50 rounded-2xl py-4 px-6 text-lg font-bold text-gray-900 outline-none focus:ring-4 focus:ring-purple-50 focus:border-purple-200 transition-all" 
                  />
               </div>
            </div>
            ...
            <div className="space-y-4 relative">
               ...
               <button 
                 ...
                 className="absolute left-[-23px] top-[48px] z-10 hidden lg:flex w-[46px] h-[46px] bg-white border border-gray-100 rounded-full items-center justify-center shadow-lg text-[#7C3AED] hover:bg-purple-50 transition-colors cursor-pointer"
               >
         ...
         <div className="flex flex-col md:flex-row gap-6 pt-6 border-t border-gray-50">
            <div className="flex-1 flex gap-4 overflow-x-auto no-scrollbar">
               {['Premium Economy', 'Business Class', 'First Class'].map((tier) => (
                  <label key={tier} className={cn(
                    "flex items-center gap-3 px-6 py-3 rounded-xl cursor-pointer hover:bg-gray-100 transition-all border border-transparent",
                    cabinClass === tier ? "border-purple-200 bg-purple-50/50" : "bg-gray-50"
                  )}>
                     <input 
                       type="radio" 
                       name="class_tier" 
                       checked={cabinClass === tier}
                       onChange={() => setCabinClass(tier)}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-12 gap-8 animate-fade-in animate-delay-1">
         <div className="lg:col-span-8 bg-gradient-to-br from-[#7C3AED] to-[#5B21B6] rounded-[3rem] p-12 text-white relative overflow-hidden group shadow-xl">
            <div className="relative z-10 max-w-lg space-y-6">
               ...
               <button 
                 type="button" 
                 onClick={() => triggerToast('Lounge Access Code: OMNI_GOLD_9012 activated!')}
                 className="bg-white text-[#7C3AED] hover:bg-gray-50 px-8 py-3 rounded-xl text-sm font-bold shadow-lg mt-2 cursor-pointer active:scale-95 transition-all"
               >
      ...
      <section className="space-y-8 animate-fade-in animate-delay-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {trendingList.map((place) => (
            <motion.div
              key={place.id}
              whileHover={{ y: -8 }}
              onClick={() => setSelectedDestination(place)}
              className="group bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-sm hover:shadow-xl transition-all cursor-pointer"
            >
              <div className="relative h-64 overflow-hidden rounded-[2rem] mb-6 shadow-sm">
                <img 
                  src={place.image} 
                  alt={place.name}
                  className="w-full h-full object-cover group-hover:scale-110 transition-all duration-[1s]"
                />
      ...
      <AnimatePresence>
        {showFlightSearch && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4 overflow-y-auto">
            <motion.div 
              ...
              {!seatMapStep ? (
                <>
                  ...
                  <div className="space-y-4 max-h-[320px] overflow-y-auto pr-1 no-scrollbar">
                    {getTransportList().map((fl) => (
                      <div key={fl.id} className="p-5 bg-gray-50 border border-gray-150 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-purple-300 transition-all shadow-sm hover:shadow-md">
                         <div className="space-y-1">
                         ...
                         <div className="text-right flex md:flex-col items-center justify-between md:justify-end gap-3 shrink-0">
                           ...
                           <button 
                             onClick={() => handleBookFlight(fl)}
                             className="px-5 py-2.5 bg-[#7C3AED] hover:bg-purple-700 text-white text-[10px] font-extrabold uppercase tracking-widest rounded-xl transition-all cursor-pointer shadow-sm active:scale-95 flex items-center gap-1.5"
                           >
                ...
                <div className="space-y-6">
                  ...
                  <div className="py-2">
                    {activeTab === 'flights' && (
                      <div className="bg-gray-50 border border-gray-200 rounded-[2.5rem] p-6 max-w-[340px] mx-auto shadow-inner relative overflow-hidden">
                        ...
                        <div className="flex flex-col items-center gap-2">
                          {[1, 2, 3, 4, 5, 6].map((rowNum) => {
                            ...
                            return (
                              <div key={rowNum} className="flex items-center gap-2.5">
                                {letters.slice(0, 2).map((letter) => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      type="button"
                                      disabled={isBlock}
                                      onClick={() => setChosenSeat(sCode)}
                                      className={cn(
                                        "w-9 h-9 rounded-xl font-mono text-[11px] font-black transition-all flex items-center justify-center cursor-pointer active:scale-95 border",
                                        isBlock ? "bg-gray-200 text-gray-450 border-gray-250 cursor-not-allowed" :
                                        isSel ? "bg-[#7C3AED] text-white border-[#7C3AED] scale-105 shadow-md shadow-purple-150" :
                                        "bg-white border-gray-250 text-gray-700 hover:border-purple-400 hover:bg-purple-50"
                                      )}
                                    >
                                      {sCode}
                                    </button>
                    ...
                    {activeTab === 'trains' && (
                      <div className="bg-gray-50 border border-gray-200 rounded-[2.5rem] p-6 max-w-[340px] mx-auto shadow-inner">
                          ...
                          {['LB-1', 'MB-2', 'UB-3', 'SL-4', 'SU-5', 'LB-6', 'MB-7', 'UB-8', 'SL-9', 'SU-10', 'LB-11', 'UB-12'].map((berth) => {
                            ...
                            return (
                              <button
                                key={berth}
                                disabled={isBlock}
                                onClick={() => setChosenSeat(berth)}
                                className={cn(
                                  "h-11 rounded-xl flex items-center justify-between px-3 text-xs font-black font-mono transition-all border",
                                  isBlock ? "bg-gray-200 text-gray-400 border-gray-200 cursor-not-allowed" :
                                  isSel ? "bg-[#7C3AED] text-white border-[#7C3AED] scale-[1.02] shadow-md shadow-purple-150" :
                                  "bg-white border-gray-250 text-gray-700 hover:bg-purple-50 hover:border-purple-300"
                                )}
                              >
                                <span>{berth}</span>
                          ...
                          {[
                            ...
                            return (
                              <button
                                key={bCode}
                                disabled={isBlock}
                                onClick={() => setChosenSeat(bCode)}
                                className={cn(
                                  "h-11 rounded-xl flex items-center justify-between px-3 text-xs font-black font-mono transition-all border",
                                  isBlock ? "bg-gray-200 text-gray-400 border-gray-250 cursor-not-allowed" :
                                  isSel ? "bg-[#7C3AED] text-white border-[#7C3AED] scale-[1.02] shadow-md shadow-purple-150" :
                                  "bg-white border-gray-250 text-gray-700 hover:bg-purple-50 hover:border-purple-300"
                                )}
                              >
                  ...
                  <div className="bg-gray-50 p-5 rounded-2xl border border-gray-100 flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <button
                      ...
                      disabled={!chosenSeat}
                      className="w-full md:w-auto px-6 h-12 bg-purple-650 hover:bg-purple-700 disabled:bg-purple-200 text-white font-black text-xs uppercase tracking-widest rounded-xl shadow-lg transition-all transform active:scale-95 text-center flex items-center justify-center gap-2 cursor-pointer"
                    >
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
            ...
            >
              ...
              <button 
                ...
                className="absolute top-6 right-6 w-10 h-10 bg-gray-50 text-gray-500 hover:text-[#7C3AED] rounded-full flex items-center justify-center border border-gray-100 cursor-pointer"
              >
                <X size={18} />
              </button>
              ...
              <button 
                onClick={() => handleBookPackage(selectedDestination)}
