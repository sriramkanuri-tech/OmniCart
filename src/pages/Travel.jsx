...
const FLIGHTS = [
  { id: "EK-503", carrier: "Emirates", dep: "10:30 AM", arr: "02:45 PM", price: 42e3, duration: "4h 15m" },
  { id: "AI-101", carrier: "Air India", dep: "06:00 AM", arr: "11:15 AM", price: 18500, duration: "5h 15m" },
  { id: "SQ-421", carrier: "Singapore Airlines", dep: "09:15 PM", arr: "03:30 AM", price: 34e3, duration: "6h 15m" }
];
const TRAINS = [
        ...
        if (res.data && res.data.length > 0) {
          ...
          if (travelProducts.length > 0) {
            const customTravelList = travelProducts.map((p) => ({
              ...
              rate: parseFloat(String(p.rate || p.price).replace(/[^0-9.]/g, "")) || 45e3,
              rating: p.rating || "4.8",
              image: p.image
            }));
  ...
  />
         ...
         <div className="flex flex-col md:flex-row gap-6 pt-6 border-t border-gray-50">
            <div className="flex-1 flex gap-4 overflow-x-auto no-scrollbar">
               {["Premium Economy", "Business Class", "First Class"].map((tier) => <label key={tier} className={cn(
  ...
  >
    ...
    /* Popular Destinations */
      ...
      <section className="space-y-8 animate-fade-in animate-delay-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {trendingList.map((place) => <motion.div
    key={place.id}
    whileHover={{ y: -8 }}
    ...
    >
      ...
      return <div key={rowNum} className="flex items-center gap-2.5">
        ...
        return <button
          ...
          className={cn(
            ...
            isBlock ? "bg-gray-200 text-gray-450 border-gray-250 cursor-not-allowed" : isSel ? "bg-[#7C3AED] text-white border-[#7C3AED] scale-105 shadow-md shadow-purple-150" : "bg-white border-gray-250 text-gray-700 hover:border-purple-400 hover:bg-purple-50"
          )}
        >
                                      {sCode}
                                    </button>;
      ...
      return <button
        ...
        className={cn(
          ...
          isBlock ? "bg-gray-200 text-gray-400 border-gray-200 cursor-not-allowed" : isSel ? "bg-[#7C3AED] text-white border-[#7C3AED] scale-[1.02] shadow-md shadow-purple-150" : "bg-white border-gray-250 text-gray-700 hover:bg-purple-50 hover:border-purple-300"
        )}
      >
                                <span>{berth}</span>
      ...
      return <button
        ...
        className={cn(
          "h-11 rounded-xl flex items-center justify-between px-3 text-xs font-black font-mono transition-all border",
          isBlock ? "bg-gray-200 text-gray-400 border-gray-250 cursor-not-allowed" : isSel ? "bg-[#7C3AED] text-white border-[#7C3AED] scale-[1.02] shadow-md shadow-purple-150" : "bg-white border-gray-250 text-gray-700 hover:bg-purple-50 hover:border-purple-300"
        )}
      >
                                <span>{bCode}</span>
                                <span className="text-[8px] font-bold opacity-75">
