...
import { 
  Search, ShoppingBag, Heart, Star, Tag, Filter, ChevronDown, 
  Grid2X2, SlidersHorizontal, X, CheckCircle2, Plus, Edit2, Sparkles
} from 'lucide-react';
    ...
    const fetchStorefrontData = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) {
          const shoppingItems = res.data.filter((p: any) => 
            p.category === 'Watches' || 
            p.category === 'Audio' || 
            p.category === 'Beauty' || 
  ...
  return (
    <div className="space-y-12 pb-12 font-sans relative">
      ...
      <div className="flex flex-col gap-8">
        <div className="flex justify-between items-end">
          ...
          <div className="flex gap-4">
             <button 
               onClick={() => triggerToast('Search filtering parameters updated!')}
               className="w-12 h-12 bg-white rounded-2xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#7C3AED] transition-colors cursor-pointer"
             >
               <Grid2X2 size={20} />
             </button>
             <button 
               onClick={() => triggerToast('Parameters sorted based on review indexes!')}
               className="w-12 h-12 bg-white rounded-2xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#7C3AED] transition-colors cursor-pointer"
             >
      ...
      <section className="relative h-[350px] rounded-[3rem] overflow-hidden group shadow-xl">
         ...
         <div className="relative z-10 h-full p-16 flex flex-col justify-center text-white space-y-6">
            ...
            <Button 
              ...
              className="w-fit h-14 px-8 rounded-2xl bg-white text-gray-900 hover:bg-gray-100 font-bold cursor-pointer transition-all"
            >
              Shop Selection
            </Button>
         </div>
      ...
      <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
        {['All Items', 'Watches', 'Audio', 'Beauty', 'Furniture'].map((cat, i) => (
          <button 
            key={i} 
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {filteredProducts.map((product) => (
            <motion.div
              ...
              className="bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-sm hover:shadow-xl transition-all cursor-pointer group flex flex-col h-full"
            >
              <div className="h-64 relative rounded-[2rem] overflow-hidden mb-6 shadow-sm">
                ...
                {product.tag && (
                  <div className="absolute bottom-4 left-4 px-4 py-2 bg-gray-900/80 backdrop-blur text-white font-bold text-[10px] uppercase tracking-widest rounded-lg">
                    {product.tag}
                  </div>
                )}
              </div>
      ...
      <section className="space-y-6">
        <div className="flex justify-between items-center">
          ...
          {isAdmin && (
            <button
              onClick={() => window.location.href = '/admin/offers?openAdd=true'}
              className="flex items-center gap-1.5 bg-purple-50 hover:bg-purple-100 text-[#7C3AED] font-bold px-4 py-2.5 rounded-xl text-xs transition-colors cursor-pointer"
            >
              <Sparkles size={14} />
              <span>+ Add Offer</span>
            </button>
            ...
            >
              ...
              <div className="space-y-2 border-t border-gray-100 pt-4">
                ...
                {showNewAddressForm ? (
                  ...
                  <div>
                    {addresses.length === 0 ? (
                      <div className="text-[10px] text-yellow-600 bg-yellow-50 p-3 rounded-xl border border-yellow-100 font-medium">
