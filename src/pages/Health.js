...
import { HeartPulse, Activity, Stethoscope, Droplets, FlaskConical, ArrowRight, ShieldCheck, Zap, Calendar, X, CheckCircle2, Watch } from 'lucide-react';
...
export default function Health() {
  ...
  const SERVICE_OFFERINGS = [{
    ...
    desc: 'Direct 24/7 access to world-class private specialists.',
    price: 12000,
    color: 'bg-purple-50 text-[#7C3AED]'
  }, {
    icon: FlaskConical,
    label: 'Full Diagnostic Lab',
    desc: 'Precise screening profile diagnostic kits from home.',
    price: 2499,
    color: 'bg-blue-50 text-blue-600'
  }, {
    icon: Droplets,
    label: 'IV Recovery Fluids',
    desc: 'Optimized hydrating solutions for instant renewal.',
    price: 1500,
    color: 'bg-cyan-50 text-cyan-600'
  }, {
    icon: ShieldCheck,
    label: 'Whole Body Checkup',
    desc: 'Supervised comprehensive physical wellness screens.',
    price: 3999,
  ...
  return <div className="space-y-12 pb-12 font-sans relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <button onClick={() => setShowAppointment(true)} className="flex items-center gap-2 px-6 py-3 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-2xl font-bold text-sm shadow-lg shadow-purple-150 transition-all cursor-pointer active:scale-95">
           <Calendar size={18} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         <div className="lg:col-span-2 bg-[#F1F4FF] rounded-[3rem] p-12 border border-blue-100 relative overflow-hidden group shadow-sm">
            <div className="relative z-10 space-y-6">
                ...
                <Button onClick={() => setShowReport(true)} className="h-14 px-8 rounded-2xl bg-[#7C3AED] hover:bg-[#6D28D9] text-white font-bold shadow-xl shadow-purple-100 cursor-pointer">
         ...
         <div className="bg-[#FFF5F1] rounded-[3rem] p-10 border border-orange-100 space-y-8 shadow-sm flex flex-col justify-between">
            ...
            <button onClick={() => setShowMeditation(true)} className="text-sm font-bold text-orange-600 flex items-center gap-2 group cursor-pointer w-fit">
      ...
      <section className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {SERVICE_OFFERINGS.map((service, i) => <motion.div key={i} whileHover={{
          y: -8
        }} onClick={() => setSelectedService(service)} className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all group cursor-pointer flex flex-col justify-between h-full">
               <div>
      ...
      <section className="bg-gray-900 rounded-[3.5rem] p-12 text-white relative overflow-hidden group shadow-2xl">
         <div className="relative z-10 max-w-2xl space-y-6">
            ...
            {activeDevice ? <div className="p-6 bg-white/5 border border-white/10 backdrop-blur rounded-2xl w-fit flex items-center gap-4">
            ...
            triggerToast('Wearable disconnected.');
          }} className="text-xs text-red-400 font-bold hover:underline ml-6">Disconnect</button>
              </div> : <div className="flex flex-wrap gap-4 pt-4">
                 <button onClick={() => triggerToast('OmniHealth app installer downloaded in sandbox memory.')} className="h-14 px-8 bg-white text-gray-900 rounded-2xl font-bold hover:bg-gray-100 transition-all cursor-pointer">
                   Download App
                 </button>
                 <button onClick={() => setActiveDevice('Apple Watch Ultra')} className="h-14 px-8 bg-[#7C3AED] text-white rounded-2xl font-bold hover:bg-purple-600 transition-all border border-purple-500/30 backdrop-blur flex items-center gap-2 cursor-pointer">
                   <Watch size={18} /> Connect Apple Watch
                 </button>
                 <button onClick={() => setActiveDevice('Fitbit sense')} className="h-14 px-8 bg-white/10 text-white rounded-2xl font-bold hover:bg-white/20 transition-all border border-white/10 backdrop-blur cursor-pointer">
                   Connect Fitbit
                 </button>
              </div>}
         </div>
      ...
      <AnimatePresence>
        {showAppointment && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
            <motion.div initial={{
          scale: 0.9,
          opacity: 0
        }} animate={{
          ...
          opacity: 0
              ...
              <button onClick={() => setShowAppointment(false)} className="absolute top-6 right-6 w-10 h-10 bg-gray-50 text-gray-500 hover:text-gray-950 rounded-full flex items-center justify-center border border-gray-100 cursor-pointer">
                <X size={18} />
              </button>
              ...
              <form onSubmit={handleBookAppt} className="space-y-4">
                <div className="space-y-2">
                  ...
                  <select value={specialist} onChange={e => setSpecialist(e.target.value)} className="w-full bg-gray-50 border border-gray-100 p-4 rounded-xl text-sm font-bold outline-none font-sans">
                ...
                <div className="space-y-2">
                  ...
                  <select value={apptDate} onChange={e => setApptDate(e.target.value)} className="w-full bg-gray-50 border border-gray-100 p-4 rounded-xl text-sm font-bold outline-none font-sans">
      ...
      <AnimatePresence>
        {showReport && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
            <motion.div initial={{
          scale: 0.9,
          opacity: 0
        }} animate={{
          ...
          opacity: 0
              ...
              <button onClick={() => setShowReport(false)} className="absolute top-6 right-6 w-10 h-10 bg-gray-50 text-gray-500 hover:text-gray-950 rounded-full flex items-center justify-center border border-gray-100 cursor-pointer">
                <X size={18} />
              </button>
              ...
              remark: 'Optimal',
              range: '65 - 90 ms',
              color: 'w-[85%] bg-green-500'
            }, {
              name: 'Sleep efficiency score',
              value: '89%',
              remark: 'Excellent',
              range: '80% - 92%',
              color: 'w-[89%] bg-blue-500'
            }, {
              name: 'Blood Oxygen (SpO2)',
              value: '98.8%',
              remark: 'Perfect',
              range: '95% - 100%',
              color: 'w-[98%] bg-cyan-500'
            }, {
              name: 'Cardio Fitness (VO2 Max)',
              value: '47.5 ml/kg',
              remark: 'Enviable',
              range: '42 - 50 ml/kg',
              color: 'w-[92%] bg-purple-500'
            }, {
              name: 'Cortisol level estimate',
              value: '8.4 μg/dL',
              remark: 'Reduced',
              range: '5.0 - 15.0 μg/dL',
              color: 'w-[56%] bg-orange-500'
            }].map((marker, idx) => <div key={idx} className="p-4 bg-gray-50 rounded-2xl border border-gray-100 flex flex-col gap-2">
      ...
      <AnimatePresence>
        {showMeditation && <div className="fixed inset-0 bg-black/90 backdrop-blur-md z-[95] flex items-center justify-center p-4">
            <motion.div initial={{
          opacity: 0
        }} animate={{
          opacity: 1
          ...
          opacity: 0
              ...
              ease: "easeInOut"
                ...
                <div className="relative z-10 flex flex-col justify-center text-white">
                  <motion.p key={breathText} initial={{
                scale: 0.8,
                opacity: 0
              }} animate={{
                scale: 1,
      ...
      <AnimatePresence>
        {selectedService && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
            <motion.div initial={{
          scale: 0.9,
          opacity: 0
        }} animate={{
          ...
          opacity: 0
              ...
              <button onClick={() => setSelectedService(null)} className="absolute top-6 right-6 w-10 h-10 bg-gray-50 text-gray-500 hover:text-gray-950 rounded-full flex items-center justify-center border border-gray-100 cursor-pointer">
                <X size={18} />
              </button>
              ...
              <button onClick={() => handleBuyService(selectedService)} className="w-full h-14 bg-[#7C3AED] hover:bg-purple-700 text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer">
