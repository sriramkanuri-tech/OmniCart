...
import { 
  CreditCard, QrCode, Plus, Send, Landmark, History, 
  Search, ShieldCheck, ArrowUpRight, ArrowDownLeft,
  ChevronRight, Award, Banknote, ShoppingBag, MapPin, 
  TrendingUp, ArrowRight, Wallet, Filter, MoreHorizontal, Utensils,
  CheckCircle2, XCircle, AlertTriangle, X, Camera, Link as LinkIcon, Lock
} from 'lucide-react';
...
export default function WalletPage() {
  const { 
    balance, transactions, updateBalance, addTransaction,
    bankAccounts, cards, addBankAccount, addCard, isLoading
  } = useGlobalState();
  
  // Modals state
  const [activeModal, setActiveModal] = useState<null | 'add' | 'send' | 'scan' | 'withdraw' | 'link_bank' | 'link_card'>(null);
  ...
  const [pendingBankDetails, setPendingBankDetails] = useState<any>(null);
  ...
  // Notification Toast
  const [toast, setToast] = useState<{ show: boolean, msg: string, type: 'success' | 'refused' }>({
    show: false,
    msg: '',
    type: 'success'
  });
  ...
  const showToast = (msg: string, type: 'success' | 'refused' = 'success') => {
  ...
  const handleVerifyBankOtpSubmit = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  return (
    <div className="space-y-12 pb-12 font-sans relative">
      ...
      {bankAccounts.length === 0 ? (
        <div className="flex flex-col items-center justify-center mt-12 gap-8 w-full max-w-[900px] mx-auto">
          <div className="w-full rounded-[1.5rem] border border-dashed border-purple-300 bg-transparent py-20 flex flex-col items-center justify-center text-center space-y-6">
             ...
             <div className="pt-2">
                <button 
                  ...
                  className="inline-flex items-center gap-2 px-6 py-3.5 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-xl font-semibold text-sm transition-all active:scale-95 cursor-pointer"
                >
         ...
         <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 animate-fadeIn">
           ...
           <div className="lg:col-span-8 space-y-12">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                 {[
                   { type: 'send', icon: Send, label: 'Send', color: 'bg-purple-100 text-[#7C3AED]' },
                   { type: 'scan', icon: QrCode, label: 'Scan & Pay', color: 'bg-orange-100 text-orange-600' },
                   { type: 'withdraw', icon: Landmark, label: 'Withdraw', color: 'bg-blue-100 text-blue-600' },
                   { type: 'ledger', icon: History, label: 'Ledger', color: 'bg-gray-100 text-gray-900' }
                 ].map((action, i) => (
                   <button 
                     key={i} 
                     onClick={() => {
                       if (action.type === 'ledger') {
                         ...
                         setActiveModal(action.type as any);
                       }
                     }}
              ...
              <div id="ledger-transactions-section" className="space-y-8 scroll-mt-24">
                 ...
                 <div className="bg-white rounded-[3rem] border border-gray-100 shadow-sm overflow-hidden">
                   {transactions.length === 0 ? (
                     <div className="p-12 text-center text-gray-400 font-semibold text-sm">No transaction activities yet.</div>
                   ) : (
                     transactions.map((tx, i) => (
                       <div key={tx.id} className={cn(
                         "flex items-center justify-between p-8 hover:bg-gray-50 transition-all cursor-pointer group",
                         ...
                         <div className="flex items-center gap-6">
                            <div className={cn(
                              ...
                              tx.type === 'in' ? 'bg-green-100 text-green-600' : 'bg-gray-150 text-gray-700'
                            )}>
           ...
           <div className="lg:col-span-4 space-y-12">
               ...
               <div className="bg-[#7C3AED] rounded-[3rem] p-10 text-white relative overflow-hidden group shadow-xl">
                  ...
                  <div className="relative z-10 space-y-8">
                     ...
                     <div className="pt-6 border-t border-white/10">
                        <button 
                          onClick={() => showToast('Rewards center updated! Successfully claimed 500 Gold Coins.')}
                          className="w-full h-14 bg-white text-[#7C3AED] rounded-2xl font-bold text-sm shadow-xl hover:bg-gray-50 transition-all cursor-pointer"
                        >
                          Redeem Points
                        </button>
                     </div>
               ...
               <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                    {bankAccounts.map((bank) => (
                       <div key={bank.id} className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-sm flex items-center justify-between group cursor-pointer hover:border-purple-200 transition-all">
               ...
               <div className="space-y-6">
                  <div className="flex justify-between items-center px-4">
                     ...
                     <button 
                       onClick={() => setActiveModal('link_card')}
                       className="text-xs font-bold text-[#7C3AED] hover:underline cursor-pointer"
                     >
                       + Link Card
                     </button>
                  ...
                  <div className="space-y-4">
                    {cards.map((card) => (
                       <div key={card.id} className="bg-white rounded-[2rem] p-6 border border-gray-100 shadow-sm flex items-center justify-between group cursor-pointer hover:border-purple-200 transition-all">
                          <div className="flex items-center gap-4">
        ...
        {activeModal && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[90] flex items-center justify-center p-4">
            ...
            >
              ...
              {/* 1. Add Money Modal */}
              {activeModal === 'add' && (
                <form onSubmit={handleAddMoneySubmit} className="p-10 space-y-6">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <input 
                        ...
                        className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-xl font-bold text-gray-900 outline-none focus:ring-2 focus:ring-[#7C3AED]/20 hover:border-gray-200 transition-all"
                        placeholder="₹ 5,000"
                        min="1"
                      />
                      <div className="grid grid-cols-4 gap-2 pt-2">
                        {['1000', '2000', '5000', '10000'].map(val => (
                          <button 
                            ...
                            className="h-10 border border-gray-100 bg-white hover:bg-gray-50 text-xs font-bold text-gray-700 rounded-lg transition-all"
                          >
                    ...
                    <div className="space-y-1.5">
                      ...
                      {bankAccounts.length === 0 ? (
                        <div className="p-4 bg-yellow-50 border border-yellow-100 rounded-2xl text-xs text-yellow-700 font-medium flex items-center justify-between">
                        ...
                        <select 
                          ...
                          className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-[#7C3AED]/20 transition-all cursor-pointer"
                        >
                          {bankAccounts.map(b => (
                  ...
                  <button 
                    ...
                    className="w-full h-14 bg-[#7C3AED] hover:bg-[#6D28D9] disabled:bg-gray-200 text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer"
                  >
              ...
              {/* 2. Send Money Modal */}
              {activeModal === 'send' && (
                <form onSubmit={handleSendMoneySubmit} className="p-10 space-y-6">
                  <div className="text-center space-y-1">
                  ...
                  <div className="space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <input 
                        ...
                        required
                        value={sendUpi}
                        onChange={(e) => setSendUpi(e.target.value)}
                        className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-bold text-gray-800 outline-none focus:ring-2 focus:ring-[#7C3AED]/20 transition-all"
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input 
                        ...
                        className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-xl font-bold text-gray-900 outline-none focus:ring-2 focus:ring-[#7C3AED]/20 transition-all"
                        placeholder="₹ 1,500"
                        min="1"
                      />
                    </div>
                  ...
                  <button 
                    type="submit"
                    className="w-full h-14 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer"
                  >
              ...
              {/* 3. Scan & Pay Modal */}
              {activeModal === 'scan' && (
                <form onSubmit={handleScanPaySubmit} className="p-10 space-y-6">
                  <div className="text-center space-y-1">
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="h-56 bg-slate-950 rounded-3xl relative overflow-hidden flex flex-col items-center justify-center text-white p-4 border border-gray-850">
                      ...
                      <button 
                        type="button"
                        onClick={() => setScanMerchant(prev => prev === 'Blue Tokai Coffee' ? 'Swiggy Food' : 'Blue Tokai Coffee')}
                        className="absolute bottom-3 right-3 z-20 bg-white/10 hover:bg-white/20 text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded text-white backdrop-blur"
                      >
                        Switch Receiver
                      </button>
                    </div>
                  ...
                  <button 
                    type="submit"
                    className="w-full h-14 bg-orange-600 hover:bg-orange-700 text-white rounded-2xl font-bold text-sm shadow-xl flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer"
                  >
              ...
              {activeModal === 'withdraw' && (
                <form onSubmit={handleWithdrawSubmit} className="p-10 space-y-6">
                  <div className="text-center space-y-1">
                  ...
                  <button 
                    type="submit"
                    disabled={bankAccounts.length === 0}
                    className="w-full h-14 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-200 text-white rounded-2xl font-bold text-sm shadow-xl flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer"
                  >
              ...
              {activeModal === 'link_bank' && (
                !bankOtpStep ? (
                  <form onSubmit={handleLinkBankSubmit} className="p-10 space-y-6">
                    <div className="text-center space-y-1">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <select 
                          ...
                          className="w-full h-12 px-4 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold text-gray-850 focus:ring-2 focus:ring-[#7C3AED]/20"
                        >
                      ...
                      <div className="space-y-1.5">
                        ...
                        <input 
                          ...
                          className="w-full h-12 px-4 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold text-gray-900 outline-none focus:ring-2 focus:ring-[#7C3AED]/20"
                          placeholder="e.g. 501004392418"
                        />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input 
                            ...
                            className="w-full h-12 px-4 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold text-gray-950 focus:ring-2 focus:ring-[#7C3AED]/20"
                    ...
                    <button 
                      type="submit"
                      className="w-full h-14 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-2xl font-bold text-sm shadow-xl flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer"
                    >
                      <Landmark size={16} /> Send OTP Verification
                    </button>
                  </form>
                ) : (
                  <form onSubmit={handleVerifyBankOtpSubmit} className="p-10 space-y-6">
                    <div className="text-center space-y-1">
                    ...
                    <div className="space-y-4">
                      <div className="space-y-1.5">
                        ...
                        <input 
                          ...
                          className="w-full h-16 bg-gray-50 border border-gray-100 rounded-2xl text-center text-3xl font-black tracking-[0.4em] text-gray-950 outline-none focus:ring-2 focus:ring-[#7C3AED]/20 focus:bg-white transition-all transition-colors placeholder:text-gray-300"
                          placeholder="••••••"
                        />
                      </div>
                    ...
                    <div className="flex flex-col gap-3">
                      <button 
                        type="submit"
                        className="w-full h-14 bg-emerald-600 hover:bg-emerald-700 text-white rounded-2xl font-bold text-sm shadow-xl shadow-emerald-100 flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer"
                      >
              ...
              {/* 6. Link Card Modal */}
              {activeModal === 'link_card' && (
                <form onSubmit={handleLinkCardSubmit} className="p-10 space-y-6">
                  <div className="text-center space-y-1">
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      <div className="col-span-2 space-y-1.5">
                        ...
                        <input 
                          ...
                          className="w-full h-12 px-4 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold text-gray-900 outline-none focus:ring-2 focus:ring-[#7C3AED]/20"
                          placeholder="4321 0982 9912 3012"
                        />
                      </div>
                      ...
                      <div className="space-y-1.5">
                        ...
                        <select 
                          ...
                          className="w-full h-12 px-2 bg-gray-50 border border-gray-100 rounded-xl text-xs font-bold text-gray-850 focus:ring-2 focus:ring-[#7C3AED]/20"
                        >
                  ...
                  <button 
                    type="submit"
                    className="w-full h-14 bg-gray-900 hover:bg-black text-white rounded-2xl font-bold text-sm shadow-xl flex items-center justify-center gap-2 transform active:scale-95 transition-all text-center cursor-pointer"
                  >
