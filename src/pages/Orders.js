...
import { ShoppingBag, Clock, ShieldCheck, Plane, Utensils, HeartPulse, ShoppingCart, Search, X, CheckCircle2 } from 'lucide-react';
    ...
    const fetchOrders = async () => {
      ...
      try {
        ...
        const mappedOrders = res.data.map(o => ({
          ...
          amount: o.amount,
          date: new Date(o.placedAt || new Date()).toLocaleDateString(),
          status: o.status
        }));
        // Sort by newest
  ...
  return <div className="space-y-12 pb-12 font-sans relative">
      ...
      <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm flex flex-col md:flex-row gap-6 items-center">
        <div className="relative group flex-1 w-full">
          ...
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Search orders, brands or descriptions..." className="w-full bg-gray-50 border border-gray-50 rounded-2xl py-4 pl-12 pr-6 text-sm font-bold text-gray-900 outline-none focus:ring-4 focus:ring-purple-50 focus:bg-white focus:border-purple-200 transition-all placeholder:text-gray-400" />
        </div>
        ...
        <div className="flex gap-2 p-1 bg-gray-50 border border-gray-100 rounded-2xl overflow-x-auto no-scrollbar w-full md:w-auto shrink-0">
          ...
          id: 'insurance',
          label: 'Insurance'
        }].map(tab => <button key={tab.id} onClick={() => setFilterType(tab.id)} className={cn("px-5 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all cursor-pointer", filterType === tab.id ? "bg-[#7C3AED] text-white shadow-md" : "text-gray-500 hover:bg-white")}>
              {tab.label}
            </button>)}
        </div>
      </div>
      ...
      <section className="space-y-8 animate-fade-in">
        {filteredOrders.length === 0 ? <div className="bg-white rounded-[3rem] p-20 text-center border border-gray-150 shadow-sm max-w-xl mx-auto space-y-4">
             ...
             <button onClick={() => navigate('/shopping')} className="mt-4 px-6 py-3 bg-[#7C3AED] hover:bg-purple-700 text-white text-sm font-bold rounded-xl shadow-lg shadow-purple-200 transition-all cursor-pointer inline-flex items-center gap-2">
               <ShoppingBag size={16} />
               Go to Shopping
             </button>
          </div> : <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {filteredOrders.map(ord => <motion.div key={ord.id} whileHover={{
          y: -4
        }} onClick={() => setSelectedOrder(ord)} className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-xl transition-all cursor-pointer flex flex-col justify-between h-full relative overflow-hidden group">
      ...
      <AnimatePresence>
        {selectedOrder && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
          ...
          opacity: 0
              ...
              <button onClick={() => setSelectedOrder(null)} className="absolute top-6 right-6 w-10 h-10 bg-gray-50 text-gray-500 hover:text-gray-950 rounded-full flex items-center justify-center border border-gray-100 cursor-pointer">
                <X size={18} />
              </button>
              ...
              <div className="space-y-6 pt-4">
                 {[{
              label: 'Order Registered & Settle processed',
              desc: 'Secure wallet authorized settlement',
              completed: true
            }, {
              label: 'Fulfillment dispatched/verified',
              desc: 'Merchant underwrote fulfillment',
              completed: ['active', 'confirmed', 'preparing', 'in transit', 'delivered', 'scheduled'].includes(selectedOrder.status.toLowerCase())
            }, {
              label: 'Carrier Transit routing',
              desc: 'Symmetric transport tracking active',
              completed: ['active', 'preparing', 'in transit', 'delivered', 'scheduled'].includes(selectedOrder.status.toLowerCase()) && selectedOrder.status !== 'confirmed'
            }, {
              label: 'Success Clearance achieved',
              desc: 'Delivered securely to consumer node',
              completed: ['delivered', 'active', 'success'].includes(selectedOrder.status.toLowerCase())
            }].map((step, sIdx) => <div key={sIdx} className="flex gap-4 relative">
                     {sIdx < 3 && <div className={cn("absolute left-[11px] top-6 w-[2px] h-[calc(100%+12px)] transition-all", step.completed ? "bg-emerald-500" : "bg-gray-150")} />}
                     <div className={cn("w-6 h-6 rounded-full flex items-center justify-center shrink-0 z-10 text-white", step.completed ? "bg-emerald-500 shadow-md shadow-emerald-100" : "bg-gray-150")}>
              ...
              <div className="border-t border-gray-100 pt-5 flex justify-between items-center text-xs font-bold leading-none select-none">
                ...
                <button onClick={() => setSelectedOrder(null)} className="px-6 py-3 bg-[#7C3AED] hover:bg-purple-700 text-white text-xs font-bold uppercase rounded-xl shadow-lg transition-all">
                  Dismiss Track
                </button>
              </div>
            </motion.div>
