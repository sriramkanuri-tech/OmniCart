...
import { 
  Smartphone, Zap, Droplets, Tv, Wifi, Phone, CreditCard, 
  ArrowRight, Search, ChevronRight, History, Plus, X, CheckCircle2, AlertTriangle,
  Edit2
} from 'lucide-react';
...
const BILL_CATEGORIES = [
  { id: 'mobile', icon: Smartphone, label: 'Mobile Recharge', color: 'bg-blue-50 text-blue-600', placeholder: 'Enter 10-digit mobile number' },
  { id: 'electricity', icon: Zap, label: 'Electricity', color: 'bg-orange-50 text-orange-600', placeholder: 'Enter 12-digit consumer account ID' },
  { id: 'water', icon: Droplets, label: 'Water Bill', color: 'bg-cyan-50 text-cyan-600', placeholder: 'Enter consumer billing number' },
  { id: 'dth', icon: Tv, label: 'DTH / Cable', color: 'bg-red-50 text-red-600', placeholder: 'Enter viewing smart-card identifier' },
  { id: 'broadband', icon: Wifi, label: 'Broadband', color: 'bg-purple-50 text-[#7C3AED]', placeholder: 'Enter fixed-line broadband number' },
  { id: 'landline', icon: Phone, label: 'Landline', color: 'bg-green-50 text-green-600', placeholder: 'Enter landline number with STD scale' },
  { id: 'card', icon: CreditCard, label: 'Credit Card', color: 'bg-gray-50 text-gray-900', placeholder: 'Enter 16-digit visual card value' },
];

export default function BillsPage() {
  const { balance, updateBalance, addTransaction, profile } = useGlobalState();
  ...
  const [selectedBiller, setSelectedBiller] = useState<any | null>(null);
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="relative w-full md:w-96">
           ...
           <input 
             ...
             className="w-full h-14 pl-14 pr-6 bg-white border border-gray-100 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] transition-all outline-none shadow-sm"
           />
        </div>
      </div>
        ...
        <div className="lg:col-span-8">
           <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {filteredCategories.map((cat, i) => (
                <motion.button 
                  key={cat.id}
                  whileHover={{ y: -5 }}
                  onClick={() => setSelectedBiller(cat)}
                  className="relative bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all group flex flex-col items-center gap-4 cursor-pointer"
                >
                   {isAdmin && (
                     <div 
                       onClick={(e) => {
                       ...
                       className="absolute top-4 left-4 bg-purple-50 text-[#7C3AED] hover:text-purple-900 p-1.5 rounded-lg text-[10px] z-10 transition-all shadow-sm flex items-center cursor-pointer border border-purple-100"
                       title="Edit Biller"
                     >
                       <Edit2 size={12} />
                     </div>
              ...
              <motion.button 
                whileHover={{ y: -5 }}
                onClick={() => triggerToast('All remaining 30+ billing departments are aggregated under these primary nodes.')}
                className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm hover:shadow-xl transition-all group flex flex-col items-center gap-4 cursor-pointer"
              >
           ...
           <div className="mt-12 bg-gradient-to-br from-[#7C3AED] to-[#6D28D9] rounded-[3.5rem] p-12 text-white relative overflow-hidden group shadow-2xl">
              <div className="relative z-10 max-w-lg space-y-6">
                 ...
                 <button 
                   onClick={() => setSelectedBiller(BILL_CATEGORIES[1])}
                   className="h-14 px-8 bg-white text-[#7C3AED] rounded-2xl font-bold hover:bg-gray-100 transition-all shadow-xl cursor-pointer"
                 >
                   Pay Bill Now
                 </button>
              </div>
                 ...
                 <div className="bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm flex items-center justify-between group h-24">
                    <div className="flex items-center gap-4">
                       <div className={cn(
                         ...
                         !bsnlFeePaid ? 'bg-red-50 text-red-600' : 'bg-gray-50 text-gray-400'
                       )}>
                          <Wifi size={20} />
                       </div>
                       <div>
                          ...
                          <p className={cn(
                            ...
                            !bsnlFeePaid ? 'text-red-500' : 'text-emerald-500'
                          )}>
                    ...
                    <div className="text-right">
                       ...
                       {!bsnlFeePaid ? (
                         <button 
                           onClick={handlePayDueBsnl}
                           className="text-[10px] font-black text-[#7C3AED] uppercase tracking-widest mt-1 hover:underline cursor-pointer"
                         >
                           Pay Now
                         </button>
                       ) : (
                         <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-widest mt-1">Settled</span>
                       )}
                    </div>
                 </div>
              ...
              <form onSubmit={handleCustomBillSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-800 outline-none focus:ring-2 focus:ring-[#7C3AED]/20 transition-all font-mono"
                ...
                <div className="space-y-1.5">
                  ...
                  <input 
                    ...
                    className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-lg font-bold text-gray-900 outline-none focus:ring-2 focus:ring-[#7C3AED]/20 transition-all"
                    placeholder="₹ 499"
                    min="1"
                  />
                </div>
                ...
                <button 
                  type="submit"
                  className="w-full h-14 bg-[#7C3AED] hover:bg-purple-700 text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 flex items-center justify-center gap-2 transform active:scale-95 transition-all cursor-pointer mt-2"
                >
