...
import { Mail, MessageSquare, Send, History, X, CheckCircle2, User, Folder, AlertCircle, Terminal, Loader2, HelpCircle } from 'lucide-react';
...
export default function Support() {
  ...
  const [tickets, setTickets] = useState(PAST_TICKETS);
  ...
  return <div className="space-y-12 pb-12 font-sans relative">
           ...
           <form onSubmit={handleSubmitComplaint} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                   ...
                   <input value={senderName} onChange={e => setSenderName(e.target.value)} className="w-full bg-gray-50 border border-gray-50 rounded-2xl py-4 px-5 text-sm font-bold text-gray-950 outline-none focus:ring-4 focus:ring-purple-50 focus:bg-white focus:border-purple-200 transition-all" />
                </div>
                ...
                <div className="space-y-2">
                   ...
                   <select value={category} onChange={e => setCategory(e.target.value)} className="w-full bg-gray-50 border border-gray-50 rounded-2xl p-4 text-sm font-bold text-gray-950 outline-none focus:ring-4 focus:ring-purple-50 focus:bg-white focus:border-purple-200 transition-all font-sans">
              ...
              <div className="space-y-2">
                 ...
                 <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Enter a brief, concise subject summary..." className="w-full bg-gray-50 border border-gray-50 rounded-2xl py-4 px-5 text-sm font-bold text-gray-950 outline-none focus:ring-4 focus:ring-purple-50 focus:bg-white focus:border-purple-200 transition-all placeholder:text-gray-400" />
              </div>
              ...
              <div className="space-y-2">
                 ...
                 <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Describe what occurred. Provide transactional coordinates, damage metrics, or settlement indexes to support faster resolution..." className="w-full h-40 bg-gray-50 border border-gray-50 rounded-2xl p-5 text-xs font-semibold text-gray-950 outline-none focus:ring-4 focus:ring-purple-50 focus:bg-white focus:border-purple-200 transition-all placeholder:text-gray-400 resize-none" />
              </div>

              <button type="submit" disabled={isSending} className="w-full h-15 bg-[#7C3AED] hover:bg-purple-700 disabled:bg-gray-200 disabled:cursor-not-allowed text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 flex items-center justify-center gap-3 transform active:scale-95 transition-all text-center cursor-pointer">
        ...
        <div className="lg:col-span-5 space-y-8">
           <div className="bg-white rounded-[3rem] p-10 border border-gray-100 shadow-sm space-y-6">
              ...
              <div className="space-y-6 max-h-[480px] overflow-y-auto no-scrollbar scroll-smooth">
                 {tickets.map(tkt => <div key={tkt.id} className="p-5 bg-gray-50 hover:bg-gray-100/50 rounded-2xl border border-gray-100 space-y-4 transition-all">
                      <div className="flex justify-between items-start gap-4">
                         ...
                         <span className={cn("text-[8px] font-black uppercase tracking-widest px-2 py-0.5 rounded-full shrink-0 border", tkt.status === 'Resolved' ? "bg-emerald-50 text-emerald-600 border-emerald-100" : tkt.status === 'Dispatched' ? "bg-blue-50 text-blue-600 border-blue-100" : "bg-amber-50 text-amber-600 border-amber-100")}>
                           {tkt.status}
                         </span>
                      </div>
      ...
      <AnimatePresence>
        {isSending && <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[105] flex items-center justify-center p-4">
            <motion.div initial={{
          scale: 0.95,
          opacity: 0
        }} animate={{
          ...
          opacity: 0
              ...
              <div className="space-y-3.5 text-xs">
                 ...
                 {dispatchStep >= 1 && <motion.p initial={{
              opacity: 0
            }} animate={{
              opacity: 1
                 ...
                 {dispatchStep >= 2 && <motion.p initial={{
              opacity: 0
            }} animate={{
              opacity: 1
            }} className="text-emerald-500/85">
                 ...
                 {dispatchStep >= 3 && <motion.p initial={{
              opacity: 0
            }} animate={{
              opacity: 1
            }} className="text-emerald-500/85">
      ...
      <AnimatePresence>
        {showSuccess && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[105] flex items-center justify-center p-4">
            <motion.div initial={{
          scale: 0.9,
          opacity: 0
        }} animate={{
          ...
          opacity: 0
              ...
              <button onClick={() => setShowSuccess(false)} className="absolute top-6 right-6 w-10 h-10 bg-gray-50 text-gray-500 hover:text-gray-950 rounded-full flex items-center justify-center border border-gray-100 cursor-pointer">
                <X size={18} />
              </button>
