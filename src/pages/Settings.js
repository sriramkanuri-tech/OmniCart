...
import { User, Shield, Bell, Key, CreditCard, Save, CheckCircle2, AlertCircle, LogOut } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  // Avatar presets
  const avatarPresets = ['https://images.unsplash.com/photo-1599566150163-29194dcaad36?q=80&w=100&auto=format&fit=crop', 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?q=80&w=100&auto=format&fit=crop', 'https://images.unsplash.com/photo-1494790108377-be9c29b29330?q=80&w=100&auto=format&fit=crop', 'https://images.unsplash.com/photo-1570295999919-56ceb5ecca61?q=80&w=100&auto=format&fit=crop'];
  ...
  return <div className="space-y-12 pb-12 font-sans text-gray-900">
      {/* Toast Notification */}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
        ...
        <div className="lg:col-span-4 space-y-3">
          ...
          label: 'My Identity',
          icon: User,
          desc: 'Update public profile and contact info'
        }, {
          id: 'security',
          label: 'Security & Auth',
          icon: Shield,
          desc: 'Two-factor auth and credentials'
        }, {
          id: 'limits',
          label: 'Transaction Limits',
          icon: CreditCard,
          desc: 'Adjust daily caps and wallet rules'
        }, {
          id: 'notifications',
          label: 'Notifications',
          icon: Bell,
          desc: 'Manage alerts and newsletter digests'
        }].map(tab => {
          const Icon = tab.icon;
          const isSelected = activeTab === tab.id;
          return <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={`w-full text-left p-6 rounded-[2rem] border transition-all flex items-start gap-4 ${isSelected ? 'bg-white border-[#7C3AED] shadow-md ring-2 ring-purple-100' : 'bg-white hover:bg-gray-50 border-gray-100 shadow-sm'}`}>
          ...
          <button onClick={handleSignOut} className="w-full text-left p-5 rounded-[2rem] border transition-all flex items-start gap-4 bg-red-50/50 hover:bg-red-100 border-red-100/50 shadow-sm text-red-600 font-bold active:scale-[0.98]">
        ...
        <div className="lg:col-span-8">
          <div className="bg-white rounded-[3rem] p-10 md:p-12 border border-gray-100 shadow-sm">
            {activeTab === 'profile' && <form onSubmit={handleSaveProfile} className="space-y-8">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  ...
                  {auth.currentUser && <div className="bg-emerald-50 text-emerald-700 px-4 py-1.5 rounded-full border border-emerald-150 text-xs font-bold inline-flex items-center gap-1.5 self-start md:self-auto">
                      <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                      Cloud Synchronization Active
                    </div>}
                </div>

                {auth.currentUser && <div className="p-6 border border-purple-100 bg-purple-50/40 rounded-3xl flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex flex-wrap gap-4 items-center">
                    ...
                    <div className="flex gap-2">
                      {avatarPresets.map((preset, i) => <button key={i} type="button" onClick={() => setAvatar(preset)} className={`w-12 h-12 rounded-xl overflow-hidden border-2 transition-all ${avatar === preset ? 'border-[#7C3AED] scale-105' : 'border-gray-200 opacity-60 hover:opacity-100'}`}>
                  ...
                  <div className="pt-2">
                    ...
                    <input type="url" value={avatar} onChange={e => setAvatar(e.target.value)} className="w-full h-12 px-4 bg-gray-50 border border-gray-100 rounded-xl text-xs font-semibold placeholder:text-gray-300 outline-none text-gray-700 focus:ring-2 focus:ring-[#7C3AED]/20 focus:border-[#7C3AED] transition-all" placeholder="https://images.unsplash.com/..." />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="space-y-1.5">
                    ...
                    <input type="text" required value={name} onChange={e => setName(e.target.value)} className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-800 focus:ring-2 focus:ring-[#7C3AED]/10 focus:border-[#7C3AED] transition-all outline-none" />
                  </div>

                  <div className="space-y-1.5">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Mobile Number</label>
                    <input type="tel" required value={phone} onChange={e => setPhone(e.target.value)} className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-800 focus:ring-2 focus:ring-[#7C3AED]/10 focus:border-[#7C3AED] transition-all outline-none" />
                  </div>

                  <div className="space-y-1.5 md:col-span-2">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-widest block">Email Address</label>
                    <input type="email" required value={email} onChange={e => setEmail(e.target.value)} className="w-full h-14 px-6 bg-gray-50 border border-gray-100 rounded-2xl text-sm font-semibold text-gray-800 focus:ring-2 focus:ring-[#7C3AED]/10 focus:border-[#7C3AED] transition-all outline-none" />
                  </div>
                </div>

                <div className="pt-4 border-t border-gray-50 flex justify-end">
                  <button type="submit" className="flex items-center gap-2 px-8 py-4 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 select-none transition-all active:scale-95">
                    <Save size={16} />
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between p-6 bg-gray-50 rounded-2xl">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input type="checkbox" checked={twoFactor} onChange={() => {
                  ...
                  <div className="flex items-center justify-between p-6 bg-gray-50 rounded-2xl">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input type="checkbox" checked={biometrics} onChange={() => {
            ...
            {activeTab === 'limits' && <div className="space-y-8">
                <div>
                ...
                <div className="space-y-8">
                  <div className="p-6 bg-gray-50 rounded-3xl space-y-4">
                    ...
                    <input type="range" min="5000" max="200000" step="5000" value={transactionLimit} onChange={e => setTransactionLimit(parseInt(e.target.value))} className="w-full accent-[#7C3AED] h-1.5 bg-gray-200 rounded-lg cursor-pointer" />
                  ...
                  <div className="pt-4 border-t border-gray-50 flex justify-end">
                    <button onClick={() => triggerSuccessNotification('Daily cap settings updated!')} type="button" className="px-8 py-4 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 transition-all active:scale-95">
                      Apply Limits
                    </button>
                  </div>
                </div>
            ...
            {activeTab === 'notifications' && <div className="space-y-8">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between p-6 bg-gray-50 rounded-2xl">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input type="checkbox" checked={paymentAlerts} onChange={() => setPaymentAlerts(!paymentAlerts)} className="sr-only peer" />
                      <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#7C3AED]"></div>
                    </label>
                  </div>
                  ...
                  <div className="flex items-center justify-between p-6 bg-gray-50 rounded-2xl">
                    ...
                    <label className="relative inline-flex items-center cursor-pointer">
                      <input type="checkbox" checked={promoEmails} onChange={() => setPromoEmails(!promoEmails)} className="sr-only peer" />
                      <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#7C3AED]"></div>
                    </label>
                  </div>
                  ...
                  <div className="pt-4 border-t border-gray-50 flex justify-end">
                    <button onClick={() => triggerSuccessNotification('Notification rules applied!')} type="button" className="px-8 py-4 bg-[#7C3AED] hover:bg-[#6D28D9] text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100 transition-all active:scale-95">
                      Save Preferences
                    </button>
                  </div>
                </div>
