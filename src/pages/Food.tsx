...
import { 
  Search, MapPin, ChevronDown, Star, Clock, Banknote, Heart, 
  Utensils, ArrowRight, Filter, Grid2X2, X, CheckCircle2, ShoppingBag,
  Plus, Edit2, Sparkles
} from 'lucide-react';
...
export default function Food() {
  ...
  const [restaurantsList, setRestaurantsList] = useState<any[]>(RESTAURANTS);
    ...
    const fetchFoodData = async () => {
      try {
        ...
        if (res.data && res.data.length > 0) {
          const foodProducts = res.data.filter((p: any) => p.category === 'Food');
          if (foodProducts.length > 0) {
            const customFoodRes = foodProducts.map((p: any) => ({
              id: p.id,
              name: p.brand || p.name,
              rating: p.rating || '4.5',
  ...
  const handlePlaceOrder = (res: any) => {
  ...
  return (
    <div className="space-y-12 pb-12 font-sans relative">
      ...
      <div className="relative group max-w-2xl">
        ...
        />
        {searchQuery && (
          <button onClick={() => setSearchQuery('')} className="absolute right-6 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-900">
            <X size={18} />
          </button>
        )}
      </div>
      ...
      <section className="space-y-8">
        ...
        {filteredRestaurants.length === 0 ? (
          <div className="bg-white rounded-3xl p-16 text-center text-gray-400 font-bold border border-gray-100 shadow-sm">
            ...
            {filteredRestaurants.map((res) => (
              ...
              >
                <div className="h-56 relative overflow-hidden rounded-[2rem] mb-6 shadow-sm">
                  ...
                  {res.discount && (
                    <div className="absolute bottom-4 left-4 px-4 py-2 bg-[#7C3AED] text-white font-bold text-[10px] uppercase tracking-widest rounded-lg shadow-lg">
                      {res.discount}
                    </div>
                  )}
                </div>
