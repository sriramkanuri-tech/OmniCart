...
import { ShoppingBag, Plane, Utensils, CreditCard, Gift, Headphones, ArrowRight, Eye, EyeOff, ShieldCheck, Smartphone, Mail, Lock, User } from 'lucide-react';
...
export default function Auth() {
  ...
  return <div className="min-h-screen flex bg-white font-sans text-[#1a1a1a]">
            ...
            ease: "easeInOut"
                ...
                <div className="p-6 pt-10 h-full bg-[#f8f9fa] flex flex-col gap-4">
                  ...
                  <div className="grid grid-cols-4 gap-2 mb-2">
                     {[ShoppingBag, Plane, Utensils, CreditCard].map((Icon, i) => <div key={i} className="flex flex-col items-center gap-1">
                          <div className={cn("w-10 h-10 rounded-xl flex items-center justify-center text-white shadow-sm", i === 0 ? "bg-pink-100 text-pink-600" : i === 1 ? "bg-blue-100 text-blue-600" : i === 2 ? "bg-orange-100 text-orange-600" : "bg-indigo-100 text-indigo-600")}>
                             <Icon size={18} />
                          </div>
      ...
      <div className="flex-1 flex flex-col relative overflow-y-auto">
        <div className="absolute top-8 right-8 z-10">
           <p className="text-sm font-medium text-gray-500">
             ...
             <button onClick={() => setIsLogin(!isLogin)} className="text-[#7C3AED] font-bold hover:underline">
        ...
        <div className="flex-1 flex flex-col justify-center py-20 px-8 md:px-16 lg:px-24">
          ...
          duration: 0.5
            ...
            {/* ERROR SUMMARY */}
            {error && <div className="mb-6 p-4 rounded-2xl bg-red-50 border border-red-100 text-red-700 text-xs font-semibold leading-relaxed shadow-sm">
                ⚠️ {error}
              </div>}
            ...
            <AnimatePresence mode="wait">
              <motion.div key="password" initial={{
              opacity: 0,
              y: 10
            }} animate={{
              opacity: 1,
              ...
              ease: "easeInOut"
                ...
                <form onSubmit={handlePasswordAuth} className="space-y-6">
                  <AnimatePresence initial={false}>
                    {!isLogin && <motion.div key="fullname-field" initial={{
                    opacity: 0,
                    height: 0
                  }} animate={{
                    opacity: 1,
                    ...
                    ease: "easeInOut"
                        ...
                        <div className="pb-4">
                          <Input label="Full Name" placeholder="e.g. Aarav Sharma" value={fullName} onChange={e => setFullName(e.target.value)} icon={<User size={18} />} className="bg-gray-50/50 border-gray-100" required />
                        </div>
                      </motion.div>}
                  </AnimatePresence>

                  <Input label="Email Address" type="email" placeholder="e.g. mail@example.com" value={email} onChange={e => setEmail(e.target.value)} icon={<Mail size={18} />} className="bg-gray-50/50 border-gray-100" required />
                  <div className="space-y-3">
                    <div className="relative">
                      <Input label="Password" type={showPassword ? "text" : "password"} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} icon={<Lock size={18} />} className="bg-gray-50/50 border-gray-100" required />
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-4 top-[3.25rem] -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors">
                  ...
                  <Button type="submit" disabled={loading} className="w-full h-14 rounded-2xl bg-[#7C3AED] hover:bg-[#6D28D9] disabled:bg-purple-300 shadow-lg shadow-purple-200 text-sm font-bold uppercase tracking-widest flex items-center justify-center gap-2 group mt-2">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button type="button" onClick={handleGoogleSignIn} disabled={loading} className="h-14 px-6 rounded-2xl border border-gray-150 bg-white hover:bg-gray-100 flex items-center justify-center gap-3 text-xs font-bold text-gray-700 transition active:scale-95 shadow-sm cursor-pointer">
              ...
              <button type="button" onClick={handleGithubSignIn} disabled={loading} className="h-14 px-6 rounded-2xl border border-gray-150 bg-white hover:bg-gray-100 flex items-center justify-center gap-3 text-xs font-bold text-gray-700 transition active:scale-95 shadow-sm cursor-pointer">
