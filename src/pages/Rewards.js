...
import { Gift, Percent, Award, Ticket, Copy, Check, HeartPulse, ShieldCheck, Plane, Utensils, ShoppingCart, ShoppingBag, CheckCircle2, Sparkles, X } from 'lucide-react';
  ...
  return <div className="space-y-12 pb-12 font-sans relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 animate-fade-in">
        ...
        <div className="flex bg-white p-1 rounded-2xl border border-gray-100 shadow-sm shrink-0">
           {[{
          id: 'rewards',
          label: 'Cashbacks & Vouchers'
        }, {
          id: 'orders',
          label: `Track Orders (${orders.length})`
        }].map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id)} className={cn("px-6 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer", activeTab === tab.id ? "bg-[#7C3AED] text-white shadow-md" : "text-gray-500 hover:bg-gray-50")}>
               {tab.label}
             </button>)}
        </div>
      </div>
      ...
      <AnimatePresence mode="wait">
        {activeTab === 'rewards' ? <motion.section key="rewards_section" initial={{
        opacity: 0,
        y: 15
      }} animate={{
        opacity: 1,
        ...
        y: -15
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {rewards.map(rew => <motion.div key={rew.id} whileHover={{
            y: -4
          }} className={cn("bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-lg transition-all relative overflow-hidden flex flex-col justify-between group", rew.status === 'Claimed' && "opacity-75")}>
                  <div className="space-y-6">
                    <div className="flex justify-between items-start">
                      <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shrink-0 shadow-sm", rew.status === 'Claimed' ? "bg-gray-100 text-gray-400" : "bg-purple-50 text-[#7C3AED]")}>
                        {rew.status === 'Claimed' ? <CheckCircle2 size={20} /> : <Percent size={20} />}
                      </div>
                      <span className={cn("text-[10px] font-black px-2.5 py-1 rounded-full uppercase tracking-wider", rew.status === 'Claimed' ? "bg-gray-100 text-gray-500 border border-gray-150" : "bg-purple-50 text-[#7C3AED] border border-purple-100")}>
                        {rew.status}
                      </span>
                    </div>
                  ...
                  setSelectedReward(rew);
                        ...
                        {rew.code ? <button onClick={() => handleCopyCode(rew.id, rew.code)} className="flex items-center gap-2 px-5 h-11 bg-gray-100 hover:bg-gray-200 text-gray-700 rounded-xl font-bold text-xs text-left grow justify-center cursor-pointer transition-colors">
                            <span className="font-mono tracking-wider text-[11px] font-black">{rew.code}</span>
                            <Copy size={12} className="text-gray-450 shrink-0" />
                          </button> : <span className="text-xs text-gray-400 italic">Voucher Claimed</span>}
                      </>}
                  </div>
                </motion.div>)}
            </div>
          </motion.section> : <motion.section key="orders_section" initial={{
        opacity: 0,
        y: 15
      }} animate={{
        opacity: 1,
        ...
        y: -15
            ...
            {orders.length === 0 ? <div className="bg-white rounded-[3rem] p-16 text-center border border-gray-100 shadow-sm max-w-md mx-auto space-y-4">
                ...
                {orders.map(ord => <motion.div key={ord.id} className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm hover:shadow-md transition-all flex gap-6">
                    {ord.image ? <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 border border-gray-50 bg-gray-50">
                        <img src={ord.image} alt={ord.title} className="w-full h-full object-cover" />
                      </div> : <div className="w-16 h-16 rounded-xl shrink-0 bg-gray-55 border border-gray-100 flex items-center justify-center text-[#7C3AED]">
      ...
      <AnimatePresence>
        {selectedReward && <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
            <motion.div initial={{
          scale: 0.9,
          opacity: 0
        }} animate={{
          ...
          opacity: 0
              ...
              <button onClick={() => setSelectedReward(null)} className="absolute top-6 right-6 w-10 h-10 bg-gray-50 text-gray-500 hover:text-gray-950 rounded-full flex items-center justify-center border border-gray-100 cursor-pointer">
                <X size={18} />
              </button>
              ...
              {selectedReward.code && <div className="p-4 bg-gray-50 border border-gray-100 rounded-2xl flex items-center justify-between gap-4 font-mono select-all">
                  <span className="text-sm font-black tracking-wider text-gray-950">{selectedReward.code}</span>
                  <button onClick={() => handleCopyCode(selectedReward.id, selectedReward.code)} className="p-2 bg-white text-gray-500 hover:text-[#7C3AED] rounded-lg border border-gray-100 hover:shadow-sm">
                    <Copy size={14} />
                  </button>
                </div>}

              <button onClick={() => setSelectedReward(null)} className="w-full h-14 bg-[#7C3AED] hover:bg-purple-700 text-white rounded-2xl font-bold text-sm shadow-xl shadow-purple-100">
                Dismiss & View Chest
              </button>
            </motion.div>
          </div>}
