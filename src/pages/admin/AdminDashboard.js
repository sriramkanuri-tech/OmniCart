...
import { Users, ShoppingCart, DollarSign, Calendar, TrendingUp, RefreshCw, Award, ShieldAlert } from 'lucide-react';
...
export default function AdminDashboard() {
  ...
  const fetchStats = async () => {
    ...
    setError(null);
    try {
      // Send admin header for auth check
  ...
  if (loading) {
    return <div className="space-y-8 animate-pulse text-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6">
          {[...Array(5)].map((_, i) => <div key={i} className="h-28 bg-slate-200 rounded-2xl"></div>)}
        </div>
  ...
  return <div className="space-y-10 text-slate-800 font-sans">
      ...
      <div className="flex justify-between items-center">
        ...
        <button onClick={fetchStats} className="flex items-center gap-2 bg-white border border-slate-200 hover:border-purple-300 text-slate-700 font-bold text-xs px-4 py-2.5 rounded-2xl shadow-sm hover:text-purple-600 transition-colors cursor-pointer">
        ...
        <div className="lg:col-span-2 bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex flex-col justify-between">
          ...
          <div className="h-[320px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <RechartsBarChart data={stats?.categoryBreakdown} margin={{
              top: 10,
              right: 10,
              left: -10,
              bottom: 0
                ...
                <Tooltip formatter={value => [`₹${parseFloat(value).toLocaleString('en-IN')}`, 'Revenue']} contentStyle={{
