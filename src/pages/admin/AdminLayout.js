...
import { BarChart, Users, ShoppingBag, Tag, ShoppingCart, ArrowLeft, ShieldCheck, Menu, X } from 'lucide-react';
...
export default function AdminLayout() {
  ...
  if (!isAdmin) {
    return <div className="min-h-screen bg-slate-930 text-white flex flex-col items-center justify-center p-6 text-center">
        ...
        <button onClick={() => navigate('/')} className="bg-purple-600 hover:bg-purple-700 px-6 py-2.5 rounded-indigo font-bold text-sm">
          Return to OmniCart
        </button>
      </div>;
  }
  return <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row text-slate-800 font-sans">
      ...
      <aside className="hidden md:flex flex-col w-64 bg-[#0F172A] text-slate-200 shrink-0 border-r border-[#1E293B]">
        ...
        <div className="p-4 mx-4 my-6 bg-[#1E293B]/50 rounded-xl flex items-center gap-3 border border-[#334155]/20">
          <img src={profile?.avatar || 'https://images.unsplash.com/photo-1599566150163-29194dcaad36?q=80&w=100&auto=format&fit=crop'} className="w-10 h-10 rounded-full border border-purple-500 object-cover" alt="Admin Avatar" />
        ...
        <nav className="flex-1 px-4 space-y-1.5 overflow-y-auto">
          ...
          return <NavLink key={item.path} to={item.path} end={item.path === '/admin'} className={({
            isActive
          }) => `
        ...
        <div className="p-4 border-t border-[#1E293B]">
          <button onClick={() => navigate('/dashboard')} className="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-xl font-bold text-sm text-slate-400 hover:text-white hover:bg-slate-800 border border-slate-700/50 transition-colors cursor-pointer">
      ...
      {mobileMenuOpen && <div className="md:hidden bg-[#0F172A] text-slate-200 px-6 py-4 space-y-2 border-b border-[#1E293B] fixed w-full top-16 left-0 z-40 shadow-2xl">
          {menuItems.map(item => {
        const Icon = item.icon;
        return <NavLink key={item.path} to={item.path} end={item.path === '/admin'} onClick={() => setMobileMenuOpen(false)} className={({
          isActive
        }) => `
