...
import { Plus, Edit2, Trash2, Tag, Calendar, ToggleLeft, ToggleRight, X, Sparkles } from 'lucide-react';
...
export default function AdminOffers() {
  ...
  const handleToggleActive = async offer => {
    ...
    try {
      await axios.put(`/api/admin/offers/${offer.id}`, {
        active: nextState
      }, {
        headers: {
          'x-admin-email': 'sriramkanuri4@gmail.com'
  ...
  return <div className="space-y-8 text-slate-800 font-sans">
      ...
      {loading ? <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {[...Array(2)].map((_, i) => <div key={i} className="bg-white border border-slate-100 h-64 rounded-3xl animate-pulse" />)}
        </div> : offers.length === 0 ? <div className="bg-white border border-slate-100 rounded-3xl p-16 text-center shadow-sm max-w-xl mx-auto">
          ...
          {offers.map(off => <div key={off.id} className={`
              ...
              <div className="h-44 overflow-hidden relative bg-slate-50">
                <img src={off.bannerImage} alt={off.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                
                {/* Badge overlay */}
                {off.badge && <span className="absolute top-4 left-4 bg-emerald-600 text-white font-extrabold text-[9px] px-2.5 py-1 rounded-lg uppercase shadow">
                    {off.badge}
                  </span>}
              ...
              <div className="p-5 flex-1 flex flex-col justify-between space-y-4">
                ...
                <div className="flex justify-between items-center pt-4 border-t border-slate-50">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button onClick={() => handleToggleActive(off)} className="text-slate-400 hover:text-slate-600 transition-colors cursor-pointer" title={off.active ? 'Deactivate Offer' : 'Activate Offer'}>
                      {off.active ? <ToggleRight className="w-9 h-9 text-[#7C3AED]" /> : <ToggleLeft className="w-9 h-9 text-slate-300" />}
                    </button>

                    {/* Edit Option */}
                    <button onClick={() => openEditModal(off)} className="p-2 hover:bg-slate-100 text-slate-500 hover:text-slate-800 rounded-xl transition-colors cursor-pointer" title="Edit Campaign">
                      <Edit2 className="w-4 h-4" />
                    </button>

                    {/* Delete Option */}
                    <button onClick={() => handleDeleteOffer(off.id, off.title)} className="p-2 hover:bg-red-50 text-red-500 hover:text-red-700 rounded-xl transition-colors cursor-pointer" title="Delete Campaign">
          ...
          <div className="bg-white rounded-3xl w-full max-w-xl shadow-2xl border border-slate-100 flex flex-col justify-between max-h-[90vh]">
            ...
            <form onSubmit={handleFormSubmit} className="flex-1 overflow-y-auto p-6 space-y-4 text-xs font-bold text-slate-500">
              <div className="space-y-1">
                ...
                <input type="text" required value={formTitle} onChange={e => setFormTitle(e.target.value)} className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-800 font-medium outline-none focus:border-purple-300" placeholder="e.g. Get 10% Cashback" />
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="uppercase tracking-wider">Discount Rate/Amount *</label>
                  <input type="text" required value={formDiscount} onChange={e => setFormDiscount(e.target.value)} className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-800 font-medium outline-none focus:border-purple-300" placeholder="e.g. 10% Cashback / ₹200 OFF" />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input type="text" value={formBadge} onChange={e => setFormBadge(e.target.value)} className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-800 font-medium outline-none focus:border-purple-300" placeholder="e.g. LIMITED OFFER" />
                </div>

                <div className="space-y-1">
                  <label className="uppercase tracking-wider">Valid Until Date *</label>
                  <input type="date" required value={formValidUntil} onChange={e => setFormValidUntil(e.target.value)} className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-800 font-medium outline-none focus:border-purple-300 cursor-pointer" />
                </div>
              </div>

              <div className="space-y-1">
                <label className="uppercase tracking-wider">Promo Banner Image URL</label>
                <input type="url" value={formBannerImage} onChange={e => setFormBannerImage(e.target.value)} className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-800 font-medium outline-none focus:border-purple-300 text-xs font-mono" placeholder="e.g. https://images.unsplash.com/..." />
              </div>

              <div className="space-y-1">
                <label className="uppercase tracking-wider">Promotion Terms/Description</label>
                <textarea value={formDescription} onChange={e => setFormDescription(e.target.value)} rows={3} required className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-800 font-medium outline-none focus:border-purple-300 text-xs leading-relaxed" placeholder="e.g. Receive 10% instant coins back in your wallet instantly at successful UroPay checkout fields..." />
              </div>
