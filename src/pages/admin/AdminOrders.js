...
import { Filter, RefreshCw, ShoppingBag, ArrowUpRight } from 'lucide-react';
...
export default function AdminOrders() {
  ...
  const handleUpdateStatus = async (orderId, currentStatus) => {
    ...
    try {
      // Allow overriding statuses for testing/demo
  ...
  const filteredOrders = orders.filter(o => {
    ...
    // @ts-ignore
    let type = o.type || '';
    if (type === 'food' || type === 'Food') orderCategory = 'Food';else if (type === 'flight' || type === 'travel' || type === 'Travel') orderCategory = 'Travel';else if (type === 'bills' || type === 'Bills') orderCategory = 'Bills';
    const matchesCategory = categoryFilter === 'All' || orderCategory === categoryFilter;

    // Date range matcher
    let matchesDate = true;
    if (startDate || endDate) {
      const orderDateStr = o.timestamp || new Date().toISOString();
    ...
    // Keyword Search Email / Product Name
  ...
  return <div className="space-y-8 text-slate-800 font-sans">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <button onClick={fetchOrders} className="flex items-center gap-2 bg-white border border-slate-200 hover:border-purple-300 text-slate-700 font-bold text-xs px-4 py-2.5 rounded-2xl shadow-sm hover:text-[#7C3AED] transition-all cursor-pointer">
      ...
      <div className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 font-bold text-xs text-slate-400">
          ...
          <div className="space-y-1">
            ...
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 outline-none text-slate-700 focus:border-purple-200">
          ...
          <div className="space-y-1">
            ...
            <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 outline-none text-slate-700 focus:border-purple-200">
          ...
          <div className="space-y-1">
            ...
            <input type="date" value={startDate} onChange={e => setStartDate(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 outline-none text-slate-700 focus:border-purple-200 cursor-pointer" />
          </div>

          {/* End Date selection */}
          <div className="space-y-1">
            <label className="uppercase tracking-wider">End Date</label>
            <input type="date" value={endDate} onChange={e => setEndDate(e.target.value)} className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 outline-none text-slate-700 focus:border-purple-200 cursor-pointer" />
          </div>

          {/* Keyword Query Search bar */}
          <div className="space-y-1">
            <label className="uppercase tracking-wider">Search</label>
            <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="e.g. Gmail / Product" className="w-full bg-slate-50 border border-slate-100 rounded-xl p-3 outline-none text-slate-700 focus:border-purple-200" />
          </div>
        </div>
      </div>
      ...
      {loading ? <div className="space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              ...
              // @ts-ignore
              let type = ord.type || '';
              if (type === 'food' || type === 'Food') mappedCat = 'Food';else if (type === 'flight' || type === 'travel' || type === 'Travel') mappedCat = 'Travel';else if (type === 'bills' || type === 'Bills') mappedCat = 'Bills';
              return <tr key={ord.orderId} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="py-4.5 px-6 text-center">
                        <button onClick={() => handleUpdateStatus(ord.orderId, ord.status)} className="bg-white border border-slate-200 hover:bg-slate-50 text-slate-600 font-bold text-xs px-3 py-1.5 rounded-xl transition-all shadow-xs cursor-pointer flex items-center gap-1 mx-auto">
