...
import { 
  Plus, Edit2, Trash2, Tag, Star, Package, 
  Trash, ShieldAlert, X, AlignLeft, Info
} from 'lucide-react';
    ...
    const checkParams = async () => {
      ...
      if (window.location.search.includes('openAdd=true')) {
        ...
        if (match && match[1]) {
          ...
          const found = allProducts.find((p: any) => String(p.id) === String(prodId));
          if (found) {
            setTimeout(() => {
              openEditModal(found);
            }, 150);
    ...
    <div className="space-y-8 text-slate-800 font-sans">
          ...
          <button
            ...
            className={`
              ...
              ${activeTab === tab 
                ...
                : 'bg-white border border-slate-100 text-slate-500 hover:text-slate-800 hover:border-slate-300'
              }
            `}
          >
            {tab}
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {[...Array(4)].map((_, i) => (
            <div key={i} className="bg-white border border-slate-100 h-96 rounded-3xl animate-pulse" />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((p) => (
            <div 
              key={p.id} 
              className="bg-white rounded-3xl border border-slate-100 shadow-sm overflow-hidden flex flex-col justify-between group hover:shadow-md transition-shadow relative"
            >
              ...
              <div className="h-52 overflow-hidden relative bg-slate-50">
                ...
                {/* Optional Badge */}
                {p.badge && (
                  <span className="absolute top-4 left-4 bg-[#7C3AED] text-white font-extrabold text-[9px] px-2.5 py-1 rounded-lg uppercase shadow">
                    {p.badge}
                  </span>
                )}
              ...
              {confirmDeleteId === p.id && (
                <div className="absolute inset-0 bg-slate-900/90 backdrop-blur-xs z-10 flex flex-col items-center justify-center p-6 text-center text-white space-y-4 animate-fade-in">
                  ...
                  <div className="flex gap-2.5 w-full max-w-[200px]">
                    <button
                      ...
                      className="flex-1 bg-slate-700 hover:bg-slate-600 px-3 py-2 rounded-xl text-xs font-bold transition-all"
                    >
                      Abort
                    </button>
                    <button
                      onClick={() => handleDeleteProduct(p.id, p.name)}
                      className="flex-1 bg-red-600 hover:bg-red-700 px-3 py-2 rounded-xl text-xs font-bold transition-all"
                    >
                      Delete
                    </button>
                  </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 border border-slate-200/60 rounded-xl p-3 text-slate-800 font-medium outline-none focus:border-purple-300"
                    placeholder="e.g. Rolex / Pizza Hut"
                  />
                </div>
              </div>
