...
import { 
  Search, Download, Trash, Eye, AlignLeft, 
  X, AlertCircle, ShoppingCart, UserCheck, Inbox
} from 'lucide-react';
...
export default function AdminUsers() {
  ...
  const fetchUsers = async () => {
    ...
    try {
      ...
      const parsedUsers = (response.data || []).map((u: UserTrack) => {
        const matchingOrders = allOrders.filter((o: any) => o.userEmail === u.email);
  ...
  const handleViewOrders = async (email: string, name: string) => {
    ...
    <div className="space-y-8 text-slate-800 font-sans">
      ...
      <div className="relative group max-w-md">
        ...
        <input 
          ...
          className="w-full bg-white border border-slate-200 rounded-2xl py-3 pl-11 pr-4 outline-none text-sm font-medium focus:border-purple-300 shadow-sm transition-all text-slate-800" 
                ...
                {filteredUsers.map((user, idx) => (
                  <tr key={user.email} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="py-4 px-6 text-center">
                      <button
                        onClick={() => handleViewOrders(user.email, user.name)}
                        className="bg-white border border-slate-200 hover:border-purple-300 text-slate-600 hover:text-[#7C3AED] font-bold text-xs px-3.5 py-2 rounded-xl shadow-sm transition-all cursor-pointer flex items-center gap-1.5 mx-auto"
                      >
          ...
          <div className="bg-white rounded-3xl w-full max-w-4xl shadow-2xl border border-slate-100 flex flex-col justify-between max-h-[85vh]">
            ...
            <div className="flex-1 p-6 overflow-y-auto min-h-[300px]">
              {ordersLoading ? (
                ...
                <div className="flex flex-col items-center justify-center min-h-[250px] text-center p-6 bg-slate-50 rounded-2xl border border-slate-100">
                ...
                <div className="border border-slate-100 rounded-2xl overflow-hidden">
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-xs text-slate-600 font-medium">
                      {userOrders.map((ord) => (
                        <tr key={ord.orderId} className="hover:bg-slate-50/40">
                          ...
                          <td className="py-3.5 px-5 text-center">
                            <span className={`
                              ...
                              ${ord.status === 'PAID' ? 'bg-green-100 text-green-700' : 
