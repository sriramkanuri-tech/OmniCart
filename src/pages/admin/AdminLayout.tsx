...
import { 
  BarChart, Users, ShoppingBag, Tag, ShoppingCart, 
  ArrowLeft, ShieldCheck, LogOut, Menu, X 
} from 'lucide-react';
...
export default function AdminLayout() {
  ...
  if (!isAdmin) {
    return (
      <div className="min-h-screen bg-slate-930 text-white flex flex-col items-center justify-center p-6 text-center">
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col md:flex-row text-slate-800 font-sans">
      ...
      {mobileMenuOpen && (
        <div className="md:hidden bg-[#0F172A] text-slate-200 px-6 py-4 space-y-2 border-b border-[#1E293B] fixed w-full top-16 left-0 z-40 shadow-2xl">
          {menuItems.map((item) => {
            const Icon = item.icon;
            return (
              <NavLink
