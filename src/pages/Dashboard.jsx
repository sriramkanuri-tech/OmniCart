...
export default function Dashboard() {
  ...
  const showAlert = (msg, success = true) => {
    ...
    setTimeout(() => setAlert((prev) => ({ ...prev, show: false })), 3e3);
  };
    ...
    /* Recent Transactions list */
            ...
            <div className="space-y-4">
               ...
               <div className="space-y-3">
                 {transactions.slice(0, 4).map((tx, idx) => <div
    key={tx.id || idx}
  ...
  />
              ...
              <div className="flex gap-2 overflow-x-auto no-scrollbar py-2">
                {["All", "Pure Veg", "Biryani", "Pizza", "Burger"].map((cat, i) => <button onClick={() => navigate("/food")} key={i} className={cn(
  ...
  >
              ...
              <div className="border-t border-dashed border-gray-200 pt-6 space-y-4">
                ...
                <div className="flex justify-between text-xs font-semibold">
                  ...
                  <span className="text-gray-800 font-mono">TXN-{Math.floor(1e5 + Math.random() * 9e5)}</span>
                </div>
                <div className="flex justify-between text-xs font-semibold">
...
function WalletAction({ icon: Icon, label, onClick }) {
...
function ServiceIcon({ icon: Icon, label, sub, color, onClick }) {
...
function DealCard({ image, name, price, original, discount }) {
...
function TransactionRow({ icon, name, date, amount, color, text }) {
...
function RestaurantCard({ image, name, meta, status }) {
...
function TravelTab({ icon: Icon, label, active, onClick }) {
  return <button
    onClick={onClick}
    className={cn(
...
function HistoryRow({ icon, name, meta, date, amount }) {
...
function FeatureItem({ icon: Icon, label, sub }) {
...
function AdminStat({ label, value, trend, status, color }) {
...
function AdminLink({ icon: Icon, label }) {
