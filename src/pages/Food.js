...
import { Search, MapPin, ChevronDown, Star, Clock, Banknote, Heart, Utensils, Filter, Grid2X2, X, CheckCircle2, ShoppingBag, Plus, Edit2 } from 'lucide-react';
...
const RESTAURANTS = [{
  ...
  featuredItem: {
    name: 'Double Whopper Mighty Meal',
    rate: 350
  },
  category: 'Burgers'
}, {
  ...
  featuredItem: {
    name: 'Supreme Garlic Bread & Personal Pizza Trio',
    rate: 450
  },
  category: 'Pizza'
}, {
  ...
  featuredItem: {
    name: 'Butter Chicken & Butter Garlic Naan Platter',
    rate: 650
  },
  ...
  featuredItem: {
    name: 'Hyderabadi Chicken Biryani Pot',
    rate: 399
  },
  category: 'Biryani'
}];
        ...
        if (res.data && res.data.length > 0) {
          ...
          if (foodProducts.length > 0) {
            const customFoodRes = foodProducts.map(p => ({
              ...
              featuredItem: {
                name: p.name,
                rate: parseFloat(String(p.rate || p.price).replace(/[^0-9.]/g, '')) || 250
              },
              category: 'Pizza'
            }));
  ...
  return <div className="space-y-12 pb-12 font-sans relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8">
        ...
        <div className="flex gap-4">
           <button onClick={() => triggerToast('Search filtering parameters applied!')} className="w-12 h-12 bg-white rounded-2xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#7C3AED] transition-all cursor-pointer shadow-sm active:scale-95">
             <Filter size={20} />
           </button>
           <button onClick={() => triggerToast('Display mode changed to wide-grid!')} className="w-12 h-12 bg-white rounded-2xl border border-gray-100 flex items-center justify-center text-gray-400 hover:text-[#7C3AED] transition-all cursor-pointer shadow-sm active:scale-95">
             <Grid2X2 size={20} />
           </button>
        </div>
      </div>
      ...
      <section className="relative h-[400px] rounded-[3rem] overflow-hidden group shadow-xl">
         <img src="https://images.unsplash.com/photo-1504674900247-0877df9cc836?q=80&w=1200&auto=format&fit=crop" alt="Food Promo" className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-[5s]" />
      ...
      <div className="flex gap-4 overflow-x-auto no-scrollbar pb-2">
        {CATEGORIES.map((cat, i) => <button key={i} onClick={() => setSelectedCat(cat)} className={cn("px-8 py-3.5 rounded-2xl text-sm font-bold whitespace-nowrap transition-all cursor-pointer", selectedCat === cat ? "bg-gray-900 text-white shadow-lg" : "bg-white text-gray-500 border border-gray-100 hover:border-gray-200")}>
            {cat}
          </button>)}
      </div>
      ...
      <section className="space-y-8">
        ...
        {filteredRestaurants.length === 0 ? <div className="bg-white rounded-3xl p-16 text-center text-gray-400 font-bold border border-gray-100 shadow-sm">
            ...
            {filteredRestaurants.map(res => <motion.div key={res.id} whileHover={{
          y: -6
        }} onClick={() => setSelectedRes(res)} className="group bg-white rounded-[2.5rem] p-6 border border-gray-100 shadow-sm hover:shadow-xl transition-all cursor-pointer flex flex-col h-full">
                <div className="h-56 relative overflow-hidden rounded-[2rem] mb-6 shadow-sm">
                  <img src={res.image} alt={res.name} className="w-full h-full object-cover group-hover:scale-110 transition-all duration-700" />
      ...
      <AnimatePresence>
        {selectedRes && <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-[95] flex items-center justify-center p-4">
          ...
          opacity: 0
              ...
              {/* Close Button */}
              <button onClick={() => setSelectedRes(null)} className="absolute top-6 right-6 w-10 h-10 bg-gray-50 text-gray-500 hover:text-gray-900 rounded-full flex items-center justify-center border border-gray-100 cursor-pointer">
                <X size={18} />
              </button>
