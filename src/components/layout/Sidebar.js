...
export function Sidebar() {
  ...
  return <aside className="fixed left-0 top-20 h-[calc(100vh-80px)] w-64 bg-white border-r border-gray-100 hidden md:flex flex-col p-6 gap-6">
      <div className="space-y-4 shrink-0">
        ...
        {bankAccounts.length > 0 && <>
            <div className="p-4 bg-purple-50 rounded-2xl border border-purple-100 flex flex-col gap-1 shadow-sm">
                ...
                <span className="text-lg font-bold text-[#7C3AED] truncate">₹ {balance.toLocaleString('en-IN', {
              minimumFractionDigits: 2
            })}</span>
            </div>
            
            <button onClick={() => navigate('/wallet')} className="w-full bg-[#7C3AED] text-white text-[11px] font-bold rounded-xl py-3 hover:bg-[#6D28D9] transition-all shadow-lg shadow-purple-200">
              Add Money
            </button>
          </>}
      </div>
      ...
      <nav className="flex-1 space-y-1.5 overflow-y-auto pr-1 scrollbar-thin scrollbar-thumb-purple-100 scrollbar-track-transparent">
        {menuItems.map(item => <NavLink key={item.label} to={item.to} className={({
        isActive
      }) => cn("flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all group", isActive ? "bg-purple-50 text-[#7C3AED] font-bold" : "text-gray-500 hover:text-gray-900 hover:bg-gray-50")}>
            {({
          isActive
        }) => <>
      ...
      <div className="mt-auto space-y-1.5 pt-4 border-t border-gray-100 shrink-0">
        ...
        isActive
      }) => cn("flex items-center gap-3 px-4 py-2.5 rounded-xl transition-all group", isActive ? "text-[#7C3AED] font-bold bg-purple-50" : "text-gray-400 hover:text-gray-900 hover:bg-gray-50")}>
          <Settings size={18} />
