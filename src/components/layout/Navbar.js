...
export function Navbar() {
  ...
  return <header className="fixed top-0 w-full z-50 flex justify-between items-center px-8 md:px-12 h-20 bg-white border-b border-gray-100">
      <div className="flex items-center gap-12">
        ...
        <nav className="hidden lg:flex gap-8 ml-4">
          {horizontalNav.map(item => <NavLink key={item.label} to={item.to} className={({
          isActive
        }) => cn("text-[12px] font-bold transition-all flex items-center gap-2", isActive ? "text-[#7C3AED]" : "text-gray-400 hover:text-gray-900")}>
              {({
            isActive
          }) => <>
      ...
      <div className="flex items-center gap-4">
        ...
        <div onClick={() => navigate('/settings')} className="w-10 h-10 rounded-xl overflow-hidden border border-gray-100 cursor-pointer ml-2 hover:ring-2 hover:ring-[#7C3AED]/30 transition-all" title="Open Settings">
