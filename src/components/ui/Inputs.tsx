import React, { useId, type ButtonHTMLAttributes, type InputHTMLAttributes, forwardRef } from 'react';
  ...
  ({ className, variant = 'primary', size = 'md', ...props }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          "inline-flex items-center justify-center rounded-lg font-semibold tracking-wide transition-all active:scale-[0.98] disabled:opacity-50 disabled:pointer-events-none",
          {
  ...
  ({ className, label, error, icon, ...props }, ref) => {
    ...
    return (
      <div className="w-full space-y-2">
        {label && (
          <label htmlFor={id} className="text-sm font-medium text-on-surface-variant ml-1">
            {label}
          </label>
        )}
        <div className="relative group">
          ...
          <input
            ...
            ref={ref}
            className={cn(
              "w-full bg-surface border border-outline rounded-xl py-4 px-5 outline-none transition-all placeholder:text-on-surface-variant/30 text-on-surface focus:border-primary focus:ring-2 focus:ring-primary/10",
              icon && "pl-12",
